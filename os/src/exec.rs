...
/// Produces a `Waker` that will wake *at least* task `mask` on invocation.
///
/// Technically, this will wake any task `n` where `n % 32 == index % 32`.
fn waker_for_mask(mask: usize) -> Waker {
...
/// one not generated by `waker_for_mask`), this will cause spurious and almost
...
/// index `index`.
fn poll_task(mask: usize, future: Pin<&mut dyn Future<Output = Infallible>>) {
    match future.poll(&mut Context::from_waker(&waker_for_mask(mask))) {
        Poll::Pending => (),
...
pub unsafe fn run_tasks_with_preemption_and_idle(
    ...
    let tasks_count = futures.len();
    let tasks_mask = (1usize << tasks_count) - 1;
    WAKE_BITS.store(initial_mask & tasks_mask, Ordering::SeqCst);
    ...
    set_timer_list(timer_list, || loop {
        interrupts.scope(|| {
            ...
            let mut mask = WAKE_BITS.swap_polyfill(0, Ordering::SeqCst);
            while mask != 0 {
                let mut i = mask.trailing_zeros() as usize;
                let i_mask = wake_mask_for_index(i);
                // Note: we need to wake up *all* tasks that alias to the
                // bitmask, not just the first one
                while i < futures.len() {
                    poll_task(i_mask, futures[i].as_mut());
                    i += 32;
                }
                mask &= !i_mask;
            }
