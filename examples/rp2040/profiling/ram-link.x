/**
 * This is copied from `cortex-m-rt`, and has had:
 * - the vector table moved after .text because we need the first byte to be the
 *   first instruction
 * - changed the `_stext` and related assert to be at the start
 */
...
/* ## .text */
ASSERT(_stext + SIZEOF(.text) <= ADDR(.vector_table), "
ERROR(cortex-m-rt): The .text section can't be placed inside the .vector_table section
Set _stext to an address greater than the end of .vector_table (See output of `nm`)");

ASSERT(_stext + SIZEOF(.text) + SIZEOF(.vector_table) < ORIGIN(FLASH) + LENGTH(FLASH), "
